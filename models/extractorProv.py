...
class ViTExtractor:
    ...
    def get_keys_from_qkv(self, qkv, input_img_shape):
        patch_num = self.get_patch_num(input_img_shape)  # Should be 197 for 224x224 images
        head_num = self.get_head_num()  # 12
        embedding_dim = self.get_embedding_dim()  # 1536

        # Ensure embedding_dim is divided correctly
        head_dim = embedding_dim // head_num  # 1536 / 12 = 128

        # Reshape qkv tensor correctly
        qkv = qkv.reshape(1, patch_num, 3, head_num, head_dim)  # (1, 197, 3, 12, 128)

        print(f"qkv shape after reshape: {qkv.shape}")  # Debugging

        # Remove batch dimension
        qkv = qkv.squeeze(0)  # Now (197, 3, 12, 128)

        # Correct permutation: (patch_num, 3, head_num, head_dim) → (3, head_num, patch_num, head_dim)
        qkv = qkv.permute(1, 2, 0, 3)  # (3, 12, 197, 128)
